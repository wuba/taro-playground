...
export default function Index() {
  ...
  const DIM_CLUSTER_INDEX = 2;
  const DATA_DIM_IDX = [0, 1];
  const CENTER_DIM_IDX = [3, 4];
  // See https://github.com/ecomfe/echarts-stat
  // @ts-ignore
  const step: any = ecStat.clustering.hierarchicalKMeans(originalData, {
    clusterCount: 6,
    outputType: 'single',
  ...
  function renderItemPoint(_, api) {
    const coord = api.coord([api.value(0), api.value(1)]);
    let clusterIdx = api.value(2);
    if (clusterIdx == null || Number.isNaN(clusterIdx)) {
      clusterIdx = 0;
    }
  ...
  function renderBoundary(_, api) {
    ...
    return {
      type: 'ellipse',
      shape: {
        cx: Number.isNaN(center[0]) ? 0 : center[0],
        cy: Number.isNaN(center[1]) ? 0 : center[1],
        rx: Number.isNaN(size[0]) ? 0 : size[0] + 15,
        ry: Number.isNaN(size[1]) ? 0 : size[1] + 15
      },
      extra: {
